...
pub trait ParserT1<'a, T0>: Sized {
    ...
    fn chain_lex2<P2: ParserT0<'a>>(self, p2: P2) -> Chain30<'a, Self, P2, T0, (), ()>
    where
        //     // Self: ParserT0<'a>,
        P2: ParserT0<'a, Input = Self::Input, Error = Self::Error>,
    {
...
impl<'a, S, T, S0> ParserT1<'a, S0> for Chain30<'a, S, T, S0, (), ()>
where
    S: ParserT1<'a, S0>,
    T: ParserT0<'a, Input = S::Input, Error = S::Error>,
{
...
mod tests {
    ...
    fn test_combo() {
        ...
        let s = parser1.parse("cat2dog").unwrap();
        assert_eq!(s, "2dog");
        ...
        let (s, d) = parser4.parse("6dogs").unwrap();
        assert_eq!(s, "s");
        assert_eq!(d, 6);
